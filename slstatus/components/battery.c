...
const char *battery_with_state(const char *bat) {
  static char buf[64];
  const char *perc = battery_perc(bat); // existing function to get %
  const char *status = battery_state(
      bat); // existing function returns "Charging", "Discharging", etc.

  if (status && status[0] == '+') // if status starts with 'C' for Charging
    snprintf(buf, sizeof(buf), "%s%% ⚡", perc);
  else
    snprintf(buf, sizeof(buf), "%s%%", perc, status);

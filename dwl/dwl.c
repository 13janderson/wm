...
void applyrules(Client *c, bool map) {
  /* rule matching */
  ...
  printf("[singleton-debug] New client: %p, appid=%s, "
         "title=%s, monitor=%s\n",
         (void *)e, appid ? appid : "(null)", title ? title : "(null)",
         c->mon ? c->mon->wlr_output->name : "(null)");
    ...
    if ((!r->title || regex_match(r->title, title)) &&
      ...
      if (r->singleton) {
        wl_list_for_each(e, &fstack, flink) {
          ...
          // We match an existing client if its title matches or app id
          // matches the rule as well.
          // In this case, we do not launch the new client and instead
          ...
          if ((!r->title || regex_match(r->title, c_title)) ||
            ...
            // do we care if its visible or not? I dont think so atm
            if (strcmp(c_appid, appid) == 0) {
              existing = e;
        ...
        if (existing) {
          printf(
              "[singleton-debug] Switching to existing client: %p, appid=%s\n",
              (void *)existing,
...
static int textw_no_ansi(Monitor *m, const char *text) {
  char buffer[256];
  int wr = 0, rd = 0;
...
static void draw_ansi_segment(Monitor *m, const char *text, int *x, int height,
                              int lrpad) {
  int fg = 7, bg = 0, fmt = 0;
  uint32_t pw;
  int left_padding = 0;
    ...
    if (text[rd] == '\033' && text[rd + 1] == '[') {
      ...
      if (seq_end) {
        ...
        if (wr > 0) {
          ...
          drwl_setscheme(m->drw, (uint32_t[]){barcolors[fg], barcolors[bg],
                                              barcolors[bg]});
          drwl_text(m->drw, *x, 0, pw + left_padding, height, left_padding,
                    buffer, fmt & 1);

          if (fmt & 2) {
            drwl_setscheme(m->drw, (uint32_t[]){barcolors[fg], barcolors[fg],
                                                barcolors[fg]});
  ...
  if (wr > 0) {
    ...
    drwl_setscheme(m->drw,
                   (uint32_t[]){barcolors[fg], barcolors[bg], barcolors[bg]});
    drwl_text(m->drw, *x, 0, pw + left_padding, height, left_padding, buffer,
              fmt & 1);

    if (fmt & 2) {
      drwl_setscheme(m->drw,
                     (uint32_t[]){barcolors[fg], barcolors[fg], barcolors[fg]});
      drwl_rect(m->drw, *x + left_padding, height - 2, pw, 1, 1, 0);
    }
